...
const AdminSupportTicketManagement = () => {
  ...
  const handleView = (ticket: SupportTicket) => {
    const activeRole = sessionStorage.getItem('activeRole');
    const basePath = activeRole === 'admin' ? 'staff' : 'admin';
    navigate(
      `/${basePath}/supportticket-management/${ticket._id}?userType=${ticket.userType}`
    );
  };
...
export default AdminSupportTicketManagement;
