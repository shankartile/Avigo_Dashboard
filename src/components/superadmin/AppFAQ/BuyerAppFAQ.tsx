...
import {
    ...
    AddBuyerFAQManagementPayload,
    updateBuyerFAQManagement,
    UpdateBuyerFAQManagementPayload
} from '../../../store/AppFAQ/BuyerAppFAQSlice';
...
const BuyerAppFAQ = () => {
    ...
    const { BuyerFAQManagement, totalItems } = useSelector((state: RootState) => state.BuyerFAQManagement);
    ...
    const faqmanagementList = Array.isArray(BuyerFAQManagement)
        ? BuyerFAQManagement.map((item) => ({ ...item, id: item._id }))
        : [];
    ...
    const addBuyerFAQManagement = async () => {
        ...
        try {
            const payload: AddBuyerFAQManagementPayload = {
    ...
    const UpdateFAQManagement = async () => {
        ...
        try {
            const payload: UpdateBuyerFAQManagementPayload = {
            ...
            const result = await dispatch(updateBuyerFAQManagement(payload)).unwrap();
            setAlertType('success');
                    ...
                    <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                        <Box ml={2}>
                            <Typography variant="h5" fontWeight={500} className="font-outfit">
                                Buyer App FAQs  <Tooltip
                                    title="Manage Buyer App FAQs by adding new questions and updating existing ones."
                                    arrow
                                    slotProps={{
                                        popper: {
                                            sx: {
                    ...
                    <DialogContent>
                        ...
                        <Box display="flex" justifyContent="center" gap={6} mt={4}>
                            <Button
                                ...
                                onClick={editIndex !== null ? UpdateFAQManagement : addBuyerFAQManagement}
