...
import {
    ...
    fetchFAQManagement,
    deleteFAQManagement,
    toggleFAQManagementStatus,
    AddFAQManagementPayload,
    updateFAQManagement,
    UpdateFAQManagementPayload
} from '../../../store/CMSManagement/FAQManagementSlice';
...
const FAQManagement = () => {
    ...
    const { FAQManagement, totalItems } = useSelector((state: RootState) => state.FAQManagement);
    ...
    const faqmanagementList = Array.isArray(FAQManagement)
        ? FAQManagement.map((item) => ({ ...item, id: item._id }))
        : [];
    ...
    const validateFAQFields = () => {
        const questionError = /[a-zA-Z]/.test(newFAQManagement.question) && newFAQManagement.question.trim().length >= 5;
        const answerError = /[a-zA-Z]/.test(newFAQManagement.answer) && newFAQManagement.answer.trim().length >= 5;
    ...
    const addFAQManagement = async () => {
        ...
        try {
            const payload: AddFAQManagementPayload = {
            ...
            const result = await dispatch(createFAQManagement(payload)).unwrap();
    ...
    const UpdateFAQManagement = async () => {
        if (loading) return; // Prevent multiple submissions
        ...
        try {
            const payload: UpdateFAQManagementPayload = {
            ...
            const result = await dispatch(updateFAQManagement(payload)).unwrap();
            setAlertType('success');
            ...
            console.error("FAQ update failed:", err); // <-- Add this
            setAlertType('error');
            setAlertMessage(`Failed to update FAQ: ${err?.message || err}`);
            setShowAlert(true);
            setTimeout(() => setShowAlert(false), 3000);
        }
        finally {
    ...
    const confirmDelete = async () => {
        if (!deleteId) return;
        try {
            await dispatch(deleteFAQManagement(deleteId)).unwrap();
            setAlertType('error');
            setAlertMessage('FAQ deleted successfully.');
            dispatch(fetchFAQManagement({ page: pageIndex, limit: pageSize }));
        } catch (err) {
            setAlertType('error');
    ...
    const handleToggleStatus = async (id: string) => {
        try {
            await dispatch(toggleFAQManagementStatus(id)).unwrap();
            setAlertType('success');
                    ...
                    <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                        <Box ml={2}>
                            <Typography variant="h5" fontWeight={500} className="font-outfit">
                                FAQs  <Tooltip
                                    title="Manage the website FAQ section by adding, updating frequently asked questions."
                                    arrow
                                    slotProps={{
                                        popper: {
                                            sx: {
                    ...
                    <DialogContent>
                        ...
                        <Box display="flex" justifyContent="center" gap={6} mt={4}>
                            <Button
                                ...
                                onClick={editIndex !== null ? UpdateFAQManagement : addFAQManagement}
