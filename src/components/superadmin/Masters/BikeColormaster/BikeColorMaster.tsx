...
import { addBikeColorMaster, AddBikeColorMasterPayload, fetchBikeColorMaster, UpdateBikeColorMasterPayload, updateBikeColorMaster, deleteBikeColorMaster, toggleBikeColorMasterStatus } from '../../../../store/Masters/BikeColorMasterSlice';
import { RootState, AppDispatch } from '../../../../store/store';
import { title } from 'process';


// Type definition
type BikeColorMaster = {
    id: string;
    bike_color_name: string;
    bike_color_code: string;
}

const BikeColorMaster = () => {
    const [newbikecolor, setnewbikecolor] = useState<BikeColorMaster>({

        id: '',
        bike_color_name: '',
        bike_color_code: ''
    });
    const [selectedColor, setSelectedColor] = useState<BikeColorMaster | null>(null);
    ...
    const { BikeColorMaster: rawList = [], totalItems } = useSelector((state: RootState) => state.BikeColorMaster);
    ...
    const isFormValid =
        ...
        newbikecolor.bike_color_name.trim() !== ""
    // newbikecolor.bike_color_code.trim() !== ""





    const handleAddBikeColorMaster = async () => {
        try {
            const payload: AddBikeColorMasterPayload = {

                bike_color_name: newbikecolor.bike_color_name,
                bike_color_code: newbikecolor.bike_color_code,

            };

            const result = await dispatch(addBikeColorMaster(payload)).unwrap();
    ...
    const handleEdit = async (BikeColorMaster: any, index: number) => {
        try {
            setnewbikecolor({
                id: BikeColorMaster._id || '',
                bike_color_name: BikeColorMaster.bike_color_name || '',
                bike_color_code: BikeColorMaster.bike_color_code || '',
    ...
    const handleUpdateBikeColorMaster = async () => {
        try {
            const payload: UpdateBikeColorMasterPayload = {
            ...
            const result = await dispatch(updateBikeColorMaster(payload)).unwrap();
            setAlertType('success');
    ...
    const confirmDelete = async () => {
        ...
        try {
            await dispatch(deleteBikeColorMaster(deleteId)).unwrap();
            setAlertType('error');
    ...
    const confirmToggle = async () => {
        ...
        try {
            await dispatch(toggleBikeColorMasterStatus({
                _id: toggleUser._id,
            ...
            dispatch(fetchBikeColorMaster({ search: searchTerm, page: pageIndex, limit: pageSize }));
        } catch (err) {
            setAlertType('error');
    ...
    const handleView = (BikeColorMaster: BikeColorMaster) => {
        setSelectedColor(BikeColorMaster);
    };
    ...
    const BikeColorMasterColumns: MRT_ColumnDef<any>[] = [
        { accessorKey: 'bike_color_name', header: 'Color Name' },
            ...
            {!showForm ? (
                <>
                    ...
                    <DataTable
                        ...
                        data={ColorList}
                        columns={BikeColorMasterColumns}
                        rowCount={totalItems}
                        pageIndex={pageIndex}
                        pageSize={pageSize}
                ...
                <Dialog open={showForm} onClose={(event, reason) => {
                    ...
                    <DialogContent>
                        ...
                        <div className="p-4 grid grid-cols-1 md:grid-cols-1 gap-4">
                            <TextField
                                label="Color Title"
                                name="bike_color_name"
                                value={newbikecolor.bike_color_name}
                            ...
                            <Box display="flex" alignItems="center" gap={6}>
                                <TextField
                                    label="Color Code"
                                    name="bike_color_code"
                                    value={newbikecolor.bike_color_code}
                                    onChange={handleChange}
                                    disabled
                                ...
                                <input
                                    ...
                                    value={newbikecolor.bike_color_code}
                                    onChange={(e) =>
                                        setnewbikecolor({ ...newbikecolor, bike_color_code: e.target.value })
                                    }
                                    style={{
                                        width: '55px',
                                        height: '55px',
                        ...
                        <Box display="flex" justifyContent="center" gap={6} mt={4}>
                            <Button
                                ...
                                onClick={editIndex !== null ? handleUpdateBikeColorMaster : handleAddBikeColorMaster}
                    ...
                    <DialogContent>
                        {selectedColor && (
                            <Box display="flex" flexDirection="column" gap={2}>
                                <Typography className="font-outfit"><strong>Color Name:</strong> {selectedColor.bike_color_name}</Typography>
                                <Typography className="font-outfit"><strong>Color Code:</strong> {selectedColor.bike_color_code}</Typography>
...
export default BikeColorMaster;
