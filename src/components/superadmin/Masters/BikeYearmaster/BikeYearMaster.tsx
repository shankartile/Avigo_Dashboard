...
import { addBikeYearMaster, AddBikeYearMasterPayload, fetchBikeYearMaster, UpdateBikeYearMasterPayload, updateBikeYearMaster, deleteBikeYearMaster, toggleBikeYearMasterStatus } from '../../../../store/Masters/BikeYearMasterSlice';
...
const BikeYearMaster = () => {
    const [newyear, setnewyear] = useState<BikeYearMaster>({

        id: '',
        year: ''
    });
    const [selectedyear, setselectedyear] = useState<BikeYearMaster | null>(null);
    ...
    const { BikeYearMaster: rawList = [], totalItems } = useSelector((state: RootState) => state.BikeYearMaster);
    ...
    const handleAddBikeYearMaster = async () => {
        try {
            const payload: AddBikeYearMasterPayload = {

                year: newyear.year,
            };

            const result = await dispatch(addBikeYearMaster(payload)).unwrap();
    ...
    const handleEdit = async (BikeYearMaster: any, index: number) => {
        try {
            setnewyear({
                id: BikeYearMaster._id || '',
                year: BikeYearMaster.year || '',
    ...
    const handleUpdateBikeYearMaster = async () => {
        try {
            const payload: UpdateBikeYearMasterPayload = {

                _id: newyear.id,
                year: newyear.year,

            };
            const result = await dispatch(updateBikeYearMaster(payload)).unwrap();
            setAlertType('success');
    ...
    const confirmDelete = async () => {
        ...
        try {
            await dispatch(deleteBikeYearMaster(deleteId)).unwrap();
            setAlertType('error');
    ...
    const confirmToggle = async () => {
        ...
        try {
            await dispatch(toggleBikeYearMasterStatus({
                _id: toggleUser._id,
            ...
            dispatch(fetchBikeYearMaster({ search: searchTerm, page: pageIndex, limit: pageSize }));
        } catch (err) {
            setAlertType('error');
    ...
    const handleView = (BikeYearMaster: BikeYearMaster) => {
        setselectedyear(BikeYearMaster);
    };
    ...
    const BikeYearMasterColumns: MRT_ColumnDef<any>[] = [
                    ...
                    <DialogContent>
                        ...
                        <Box display="flex" justifyContent="center" gap={6} mt={4}>
                            <Button
                                ...
                                onClick={editIndex !== null ? handleUpdateBikeYearMaster : handleAddBikeYearMaster}
                                className="rounded-[25px]"
                                disabled={!newyear.year}
                            >
