...
const SocietyStaffForm: React.FC<StaffFormProps> = ({ onCancel, editData, isEditMode = false }) => {
    ...
    const permissionsList: Permission[] = useSelector((state: RootState) => state.societystaff.permissions);
        ...
        try {
            if (activeTab === 0) {
                ...
                if (isEditMode && editData?._id) {
                    ...
                    setAlertMessage('Society Staff updated successfully');
                    setShowAlert(true);
                    setTimeout(() => setShowAlert(false), 3000);
                } else {
                    await dispatch(addStaff(finalForm)).unwrap();
                    setAlertType('success');
                    setAlertMessage('Society Staff added successfully');
                    setShowAlert(true);
                    setTimeout(() => setShowAlert(false), 3000);
                }
            } else if (activeTab === 1) {
                    ...
                    {(!isEditMode || activeTab === 0) && (
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                {permissionsList
                                    .filter((perm) => perm.key !== 'dashboard' && perm.key !== "society-staff-management") // <-- HIDE dashboard from UI
                                    .map((perm) => (
                                        <FormControlLabel
                                            key={perm.key}
                                            control={
...
export default SocietyStaffForm;
