...
import {
  ...
  sendMessageToTicket,
  fetchTicketById
} from '../../../store/SupportticketManagement/SupportticketManagementSlice';
...
const SupportTicketDetails = () => {
  ...
  const userType = new URLSearchParams(search).get('userType') || 'Buyer';
  const MAX_FILE_SIZE = 2 * 1024 * 1024; // 2 MB in bytes

  const dispatch = useDispatch<AppDispatch>();
  const navigate = useNavigate();

  const { currentTicket: ticket } = useSelector((state: RootState) => state.supportticket);
