...
import { fetchwebsitePolicy, updatewebsitePolicy } from '../../../store/TermsandConditionPrivacyPolicy/PrivacyPolicySlice';
import Alert from '../../ui/alert/Alert';
import Skeleton from 'react-loading-skeleton';
import 'react-loading-skeleton/dist/skeleton.css';


type WebsitePrivacyPolicyForm = {
  title: string;
  description: string;
};
...
const WebsitePrivacyPolicyForm = () => {
  const dispatch = useDispatch<AppDispatch>();
  const { data: policyData} = useSelector((state: RootState) => state.privacypolicy);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!policyData?._id) return;

      await dispatch(
        updatewebsitePolicy({ id: policyData._id, payload: { title, description } })
      ).unwrap();

      setAlertType('success');
      setAlertMessage('Website Privacy Policy updated successfully.');
      setShowAlert(true);
      setTimeout(() => setShowAlert(false), 3000);
      dispatch(fetchwebsitePolicy());
    } catch (err) {
      setAlertType('error');
      setAlertMessage('Failed to update Privacy policy.');
      setShowAlert(true);
      setTimeout(() => setShowAlert(false), 3000);
    }
    finally {
...
export default WebsitePrivacyPolicyForm;
