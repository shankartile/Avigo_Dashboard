...
import { addDealerTutorialManagement, AddDealerTutorialManagementPayload, fetchDealerTutorialManagement, UpdateDealerTutorialManagementPayload, updateDealerTutorialManagement, deleteDealerTutorialManagement, toggleDealerTutorialManagementStatus } from '../../../store/TutorialManagement/DealerTutorialManagementSlice';
...
const DealerTutorialManagement = () => {
    const [newtutorial, setnewtutorial] = useState<DealerTutorialManagement>({
        id: '',
        isFullImage: false,
    ...
    const [selectedtutorial, setselectedtutorial] = useState<DealerTutorialManagement | null>(null);
    ...
    const { DealerTutorialManagement: rawList = [], totalItems } = useSelector((state: RootState) => state.Dealertutorialmanagement);
    ...
    useEffect(() => {
        const delayDebounce = setTimeout(() => {
            ...
            dispatch(fetchDealerTutorialManagement({
                search: searchTerm,
                page: pageIndex,
                limit: pageSize,
            }));
    ...
    const handleAddDealerTutorialManagement = async () => {
        ...
        try {
            const payload: AddDealerTutorialManagementPayload = {
            ...
            const result = await dispatch(addDealerTutorialManagement(payload)).unwrap();
    ...
    const handleEdit = async (DealerTutorialManagement: any, index: number) => {
        try {
            setnewtutorial({
                id: DealerTutorialManagement._id || '',
                isFullImage: DealerTutorialManagement.isFullImage ?? false,
                thumbnail_image: DealerTutorialManagement.thumbnail_image ?? null,
                title: DealerTutorialManagement.title ?? '',
                sub_title: DealerTutorialManagement.sub_title ?? '',
                full_image: DealerTutorialManagement.full_image ?? null,
                description: DealerTutorialManagement.description ?? '',
            })
            setEditIndex(index);
            setShowForm(true);
        } catch (err) {
    ...
    const handleUpdateDealerTutorialManagement = async () => {
        ...
        try {
            const payload: UpdateDealerTutorialManagementPayload = {
                _id: newtutorial.id,
                isFullImage: newtutorial.isFullImage,
                full_image: newtutorial.full_image,
                thumbnail_image: newtutorial.thumbnail_image,
            ...
            const result = await dispatch(updateDealerTutorialManagement(payload)).unwrap();
            setAlertType('success');
    ...
    const confirmDelete = async () => {
        ...
        try {
            await dispatch(deleteDealerTutorialManagement(deleteId)).unwrap();
            setAlertType('error');
    ...
    const handleView = (DealerTutorialManagement: DealerTutorialManagement) => {
        setselectedtutorial(DealerTutorialManagement);
    };
    ...
    const confirmToggle = async () => {
        ...
        try {
            await dispatch(toggleDealerTutorialManagementStatus({
                _id: toggleUser._id,
            ...
            //  Close the toggle modal after success
    ...
    const DealerTutorialManagementColumns: MRT_ColumnDef<any>[] = [
        {
...

        {
            header: 'Actions',
            Cell: ({ row }) => {
                const DealerTutorialManagement = row.original;
    ...
    return (
        <>
                ...
                <>
                    <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                        <Box ml={2}>
                            ...
                            >
                                Dealer Tutorial Management  <Tooltip
                                    title="Manage dealer app tutorial screens by adding new tutorials, deactivating old ones, and configuring up to five active images in full-image or image-with-details format."
                                    arrow
                                    slotProps={{
                                        popper: {
                                            sx: {
                    ...
                    <DataTable
                        ...
                        data={tutoriallist}
                        columns={DealerTutorialManagementColumns}
                        rowCount={totalItems}
                        pageIndex={pageIndex}
                        pageSize={pageSize}
                ...
                <Dialog open={showForm} onClose={(event, reason) => {
                    ...
                    <Box sx={{ background: 'linear-gradient( #255593 103.05%)', height: 25, p: 4, position: 'relative' }}>
                        <DialogTitle className='font-outfit' sx={{ color: 'white', position: 'absolute', top: 5 }}>
                            {editIndex !== null ? 'Edit Tutorial' : 'Add Tutorial'}
                        </DialogTitle>
                    ...
                    <DialogContent>
                        ...
                        <Box display="flex" justifyContent="center" gap={6} mt={4}>
                            <Button
                                onClick={editIndex !== null ? handleUpdateDealerTutorialManagement : handleAddDealerTutorialManagement}
            ...
            <Dialog open={!!selectedtutorial} onClose={handleCloseModal} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '25px' } }}
                BackdropProps={{
                    sx: {
                        backdropFilter: 'blur(4px)',
                        backgroundColor: 'rgba(0, 0, 0, 0.2)'
...
export default DealerTutorialManagement;
