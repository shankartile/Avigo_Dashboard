...
export const fetchTicketById = createAsyncThunk(
  ...
  async ({ id, userType }: { id: string; userType: string }, { rejectWithValue }) => {
    try {
      const endpoint =
        userType === 'Buyer'
          ? `buyer/support_ticketRoute/getbyId/${id}`
          : `dealer/support_TicketRoutes/getbyId/${id}`;
...
export const fetchsupportticket = createAsyncThunk(
  'supportticket/fetchsupportticket',
  async (
    {
      search = '',
      filters = {},
    ...
    try {
      let endpoint = type?.toLowerCase() === 'buyer'
        ? 'buyer/support_ticketRoute/getall'
        : 'dealer/support_TicketRoutes/getall';
...
export const deleteItem = createAsyncThunk(
  'supportticket/deleteItem',
  async (_id: string, { rejectWithValue }) => {
    try {
      // Sending the ID as part of the URL
      await httpinstance.delete(`buyer/support_ticketRoute/${_id}/delete`);
      return _id;
    } catch (err: any) {
...
export const updateTicketStatus = createAsyncThunk(
  ...
  async (
    ...
    try {
      const endpoint =
        userType === 'Dealer'
          ? `dealer/support_TicketRoutes/${id}/status`
          : `buyer/support_ticketRoute/${id}/status`;
...
export const addTicketNote = createAsyncThunk(
  ...
  async (
    ...
    try {
      const endpoint =
        userType === 'Dealer'
          ? `dealer/support_TicketRoutes/${id}/note`
          : `buyer/support_ticketRoute/${id}/note`;
...
export const sendMessageToTicket = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const endpoint =
        userType === 'Dealer'
          ? `dealer/support_TicketRoutes/support-tickets/${id}/messages`
          : `buyer/support_ticketRoute/support-tickets/${id}/messages`;
...
const supportticketlice = createSlice({
  name: 'supportticket',
  initialState,
  reducers: {},
...
export default supportticketlice.reducer;